...
- (BOOL)parseOptionsWithArguments:(NSArray *)arguments commandLine:(NSString *)cmd block:(GBCommandLineParseBlock)handler
{
    ...
    while (index < [arguments count]) {
        ...
        if (data == (id)GBCommandLineNotAnOptionKey)
            break; // no more options, only arguments left...
...
- (NSDictionary *)optionDataForOption:(NSString *)shortOrLongName value:(NSString **)value
{
    ...
    if (valueRange.location != NSNotFound) {
        if (value)
            *value = [name substringFromIndex:valueRange.location + 1];
...
{
    if (!self.registeredOptionGroupsByNames)
        return NO;
    if (!self.registeredOptionGroupsByNames[value])
        return NO;
    return YES;
}
